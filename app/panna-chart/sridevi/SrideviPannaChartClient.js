// app/panna-chart/kalyan/SrideviPannaChartClient.js
'use client';
...
export default function SrideviPannaChartClient({ initialIsAdmin, initialRows }) {
  ...
  const handleRowClick = (row) => {
    ...
    if (row.weekData) {
      setWeekData({
        ...
        sun: row.weekData.sat || { openPanna: '', jodi: '', closePanna: '' },
      });
    }
  ...
  const handleSeedImport = async () => {
    ...
    const confirmed = window.confirm(
      'Are you sure you want to import ALL Sridevi seed data into Firebase?'
    );
    ...
    try {
      const response = await fetch('/api/panna-chart/sridevi/seed', {
        method: 'POST',
        headers: {
  ...
  const handleSave = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/panna-chart/sridevi', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <main className="min-h-screen bg-gray-100 p-1 sm:p-3 font-sans flex flex-col items-center">
      <div className="w-full max-w-4xl mx-auto my-1 border-2 border-red-800 bg-yellow-400 p-0.5 shadow-md">
        ...
        <div className="w-full bg-white border border-black overflow-hidden">
          <div>
            <div className="grid grid-cols-[1.1fr_repeat(7,_1fr)] text-center font-black italic border-b border-black bg-white text-[10px] sm:text-sm py-1.5 uppercase">
              ...
              <div className="border-r border-black">SAT</div>
              <div>SUN</div>
            </div>
            ...
            {chartRows.length === 0 ? (
              ...
              chartRows.map((row) => (
                <div
                  ...
                  className={`grid grid-cols-[1.1fr_repeat(7,_1fr)] border-b last:border-b-0 border-gray-400 items-center ${
