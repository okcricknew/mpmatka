...
export default function MarketListClient({ initialResults }) {
  ...
  const handleNavigation = (marketName, type) => {
    const formattedSlug = marketName
      .toLowerCase()
      .replace(/\s+/g, "-");
      ...
      <div className="w-full space-y-0">
        ...
        {STATIC_MARKETS.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`w-full border-b border-gray-300 py-2.5 px-3 flex flex-col items-center box-border ${
                isHighlighted
                  ? "bg-yellow-300"
                  : "bg-white"
              }`}
            >
              ...
              <div className="w-full flex justify-between items-end">
                ...
                <div className="text-center flex-1 mx-1">
                  ...
                  <h3 className="w-full font-bold text-[25px] text-black tracking-wide">
                    {item.name}
                  </h3>

                  <p className="w-full text-red-600 font-extrabold text-[22px] tracking-widest my-0.5">
                    {displayResult}
                  </p>
                  ...
                  <span className="w-full text-[16px] text-black font-semibold">
                    ({displayTime})
                  </span>
