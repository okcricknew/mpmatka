...
const [isAdmin, setIsAdmin] = useState(false)
  ...
  const handlePostGuess = async (e) => {
    e.preventDefault()
    if (!guessText.trim() || !currentUser || !canPost) return
    ...
    <div className="w-full mt-2 bg-white font-sans text-sm">
      ...
      {canPost ? (
        ...
        >
          ...
          <div className="flex justify-between items-center mt-2">
            <span className="text-[10px] font-bold text-green-700">
              Guesser Name: <span className="text-blue-900 uppercase">
        ...
        <div className="p-3 bg-yellow-50 text-center border-b border-orange-400 text-xs text-yellow-900 font-bold">
          {currentUser
  ? "🔒 Account deactivated or missing posting permissions."
  : "⚠️ Please login to post or quote in the forum."}
        </div>
      )}
